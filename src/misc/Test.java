...
public class Test {
		...
		if (Qi == n) {// a valid configuration found.
			...
			for (int column = 0; column < n; column++) {
				if (isSafePlace(column, Qi, board)) {
					...
					/**
					 * backtracking. It is not required in this as we only look
					 * previously placed queens in isSafePlace method and it
					 * doesnot care what values are available in next
					 * positions.*
					 */
					board[Qi] = -1;
				}
	...
	private static boolean isSafePlace(int column, int Qi, int[] board) {
		...
		for (int i = 0; i < Qi; i++) {
			if (board[i] == column) { // the ith Queen(previous) is in same
										// column
				return false;
			}
			// the ith Queen is in diagonal
			// (r1, c1) - (r2, c1). if |r1-r2| == |c1-c2| then they are in
			// diagonal
